 ...
 * - 완료 후 split_46의 A1 + B/D/E/G/H열 2행 이하 초기화
 *************************************************/
...
function append_split46_to_DataDS() {
  ...
  const rawValues = src.getRange(2, 6, 46, 3).getValues();

  // 2) 완전히 빈 행 제거
  const values = rawValues.filter(r => r.some(v => String(v).trim() !== ''));
  if (values.length === 0) {
    SpreadsheetApp.getUi().alert('붙여넣을 데이터가 없습니다. (F2~H47이 모두 비어있음)');
    return;
  }
  ...
  SpreadsheetApp.getUi().alert(
    ...
    `split_46 시트의 A1 및 B/D/E/G/H열을 초기화했습니다.`
  );
}