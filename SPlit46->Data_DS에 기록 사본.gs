/*************************************************
 * split_46!F2:H47 → Data_DS!A:C 로 이어붙이기
 * - F→A, G→B, H→C
 * - Data_DS의 2행 이하가 비어있으면 2행부터, 아니면 마지막 데이터 다음 행부터
 * - 완전히 빈 행(F,G,H 모두 공백)은 건너뜀
 * - 완료 후 split_46의 B, D, E열의 2행 이하 모든 셀 내용 지우기
 *************************************************/

function append_split46_to_DataDS() {
  const ss  = SpreadsheetApp.getActive();
  const src = ss.getSheetByName('split_46');
  const dst = ss.getSheetByName('Data_DS');
  if (!src) throw new Error('split_46 시트를 찾을 수 없습니다.');
  if (!dst) throw new Error('Data_DS 시트를 찾을 수 없습니다.');

  // 1) 소스 범위: split_46!F2:H47 (46행, 3열)
  const sourceRange = src.getRange(2, 6, 46, 3); // row 2, col F(6), 46 rows, 3 cols (F,G,H)
  const rawValues = sourceRange.getValues();

  // 2) 완전히 빈 행 제거 (F,G,H 모두 공백)
  const values = rawValues.filter(r => r.some(v => String(v).trim() !== ''));
  if (values.length === 0) {
    SpreadsheetApp.getUi().alert('붙여넣을 데이터가 없습니다. (F2~H46가 모두 비어있음)');
    // 비어 있어도 요청대로 B/D/E 2행 이하 지우기는 수행 가능하다면 아래 주석 해제
    // clearSplit46ColsBDE_(src);
    return;
  }
  ...
  // 5) split_46의 B/D/E 열(2행 이하) 내용 지우기
  clearSplit46ColsBDE_(src);
  ...
  SpreadsheetApp.getUi().alert(
    ...
    `추가 후 split_46 시트의 B/D/E열 2행 이하 내용을 모두 삭제했습니다.`
  );
}

/** split_46 시트의 B, D, E 열의 2행 이하 모든 셀 내용 지우기 */
function clearSplit46ColsBDE_(sheet) {
